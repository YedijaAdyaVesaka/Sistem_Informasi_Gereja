...
@section('content')
    ...
    <div class="content-form">
        <h4 class="fw-semibold mb-4">Edit Data Pelayan</h4>
        {{-- Form --}}
        <div class="form-add p-4 bg-white rounded-4">
            <form action="{{route('pelayan.update')}}" method="POST">
                @csrf
                <input type="hidden" name="id_pelayan" value="{{ $pelayan->id }}">
                <div class="mb-4">
                    ...
                    @foreach($jadwal as $j)
                    <option value="{{$j->id}}"{{ $pelayan->id_jadwal == $j->id ? "Selected" : '' }}>{{$j->nama}}</option>
                    @endforeach
                    </select>
                </div>
                <div class="mb-4">
                    ...
                    <option value="{{$m->id}}"{{ $pelayan->id_majelis == $m->id ? "Selected" : '' }}>{{$m->nama}}</option>
                    @endforeach
                    </select>
                </div>
                <div>
